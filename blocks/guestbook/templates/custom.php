...
foreach($posts as $p) { 
	...
	<?php  if($p['approved'] || $bp->canWrite()) { 
		...
		$int_rand4 = rand(5, 30);
		?>
		<div class="guestbook_custom">
    <div class="guestBook-entry<?php  if ($c->getVersionObject()->getVersionAuthorUserName() == $u->getUserName()) {?> authorPost <?php  }?>" style="border-top-left-radius:<?php echo $int_rand1 ?>px; border-top-right-radius:<?php echo $int_rand2 ?>px; border-bottom-right-radius:<?php echo $int_rand3 ?>px; border-bottom-left-radius:<?php echo $int_rand4 ?>px;">
...
<?php  if($controller->displayGuestBookForm) { ?>
	...
	<?php  }else{ ?>	
		...
		<div id="guestBook-formBlock-<?php echo $controller->bID?>" class="guestBook-formBlock">
			<?php
 if (isset($response)) { ?>
	<?php echo "<div class=\"errors\">".$response."</div>" ?>
<?php  } ?>
			...
			<?php  if(!$controller->authenticationRequired){ ?>
				...
				<label for="email"><?php echo t('Email')?>:</label><?php echo (isset($errors['email'])?"<span class=\"error\">".$errors['email']."</span>":"")?> <span class="note">(<?php echo t('Your email will not be publicly displayed.')?>)</span> <br />
				<input type="email" name="email" value="<?php echo $Entry->user_email ?>" /><br />
			<?php  } ?>
			
			<?php echo (isset($errors['commentText'])?"<br /><span class=\"error\">".$errors['commentText']."</span>":"")?>
			<label for="commentText"><?php echo t('Message')?></label>
